...
class ZinoUserCreationForm(forms.ModelForm):
    ...
    def clean_username(self):
        ...
        try:
            ZinoUser._default_manager.get(username=username)
        except ZinoUser.DoesNotExist:
            return username
    ...
    def clean_email(self):
        ...
        # 判断是这个email 用户是否存在
        try:
            ZinoUser._default_manager.get(email=email)
        except ZinoUser.DoesNotExist:
            return email
        raise forms.ValidationError(
    ...
    def save(self, commit=True):
        user = super(ZinoUserCreationForm, self).save(commit=False)
...
class VmaigPasswordRestForm(forms.Form):
    ...
    def clean(self):
        ...
        if username and email:
            try:
                self.user = ZinoUser.objects.get(
    ...
    def save(self, from_email=None, request=None,
        ...
        try:
            send_mail(title, message, from_email, [self.user.username,self.user.email])
